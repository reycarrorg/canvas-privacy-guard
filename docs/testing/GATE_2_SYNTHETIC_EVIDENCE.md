...
The adapter also validates `parentFrameId` as an integer no smaller than `-1`. Malformed frame metadata, any delivered child-frame metadata, and suspected top-level assessment paths immediately enter `ASSESSMENT_SAFE`, detach observation, fence earlier queued work, add no activity row, and still return `undefined` with `networkAction: ALLOW`. Later same-tab update events cannot clear the in-memory assessment lock.

Exact hosted-Canvas enrollment is reconciled as a single-origin transaction. Deterministic popup/API tests hold alpha as the prior enrollment, request beta, read back beta from adapter storage, revoke alpha, and verify that only beta remains granted. Failure tests prove either that alpha's enrollment and permission are restored or that a residual grant is counted visibly while the adapter remains `UNCERTAIN_ALLOW`. Startup reconciliation removes only mismatched exact hosted-Canvas grants and any browser-reported provider wildcard grant; it preserves the enrolled exact origin and both standing synthetic hosts.
