...
The manifest declares `https://*.instructure.com/*` only as an optional permission. Opening the extension on an institution's hosted Canvas tab allows the browser to grant only that exact origin; switching origins commits and reads back the new enrollment before revoking the prior exact grant, and residual grants are surfaced rather than hidden. The extension does not read page content, answers, grades, cookies, headers, or request bodies, and suspected or unknown assessment routes suspend observation without adding an activity row. The active optional-blocking rule set is visibly **zero** until a separately hosted endpoint passes the rule-specific evidence gate.
